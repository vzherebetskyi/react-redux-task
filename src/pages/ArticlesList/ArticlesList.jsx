...
const ArticlesList = () => {
  ...
  const getNews = async (searchValue) => {
    setIsLoading(true);
    try {
      const newsArticles = await Promise.allSettled([getNewsAPINews(searchValue), getGuardianAPINews(searchValue), getNYTAPINews(searchValue)]);
      ...
      const unifiedFiltersData = {
        date: [...new Set([...unifiedNewsApiArticles[1].date.filter(el => el), ...unifiedGuardianApiArticles[1].date.filter(el => el), ...unifiedNYTApiArticles[1].date.filter(el => el)])],
        category: [...new Set([...unifiedNewsApiArticles[1].category.filter(el => el), ...unifiedGuardianApiArticles[1].category.filter(el => el), ...unifiedNYTApiArticles[1].category.filter(el => el)])],
        source: [...new Set([...unifiedNewsApiArticles[1].source.filter(el => el), ...unifiedGuardianApiArticles[1].source.filter(el => el), ...unifiedNYTApiArticles[1].source.filter(el => el)])],
      };

      dispatch(addArticles([...unifiedNewsApiArticles[0], ...unifiedGuardianApiArticles[0], ...unifiedNYTApiArticles[0]]));
  ...
  return (
    <Layout>
      ...
      <div className="articles-wrapper">
        {isLoading ? [...Array(5).keys()].map((el, idx) => <SkeletonCard key={`skeleton-${idx}`} />) : articles.length > 0 ? articles.map((article, idx) => <ArticleCard key={`${article.api}-${idx}`} article={article} />) : <p className='no-articles-message'>No articles found</p>}
      </div>
    </Layout>
  );
}
