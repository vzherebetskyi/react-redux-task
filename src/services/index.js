  ...
  try {
    ...
    if (preferredSources && preferredSources.length > 0) {
      baseQuery +=
        ...
        preferredSources
          .map((el, idx) =>
            idx !== preferredSources.length - 1
              ? el.toLowerCase().split(' ').join('-') + ','
              : el.toLowerCase().split(' ').join('-'),
          )
          .join('');
    }
...
export const getNYTAPINews = async (searchValue) => {
  try {
    ...
    if (preferredSources && preferredSources.length > 0) {
      if (
        !JSON.stringify(preferredSources).toLowerCase().includes('nyt') &&
        !JSON.stringify(preferredSources).toLowerCase().includes('new york times')
      ) {
        return { articles: [] };
      }
    }
